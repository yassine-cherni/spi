...
#include "diffdrive_arduino/stm32_spi_comms.hpp"
...
class DiffDriveArduinoHardware : public hardware_interface::SystemInterface
{
  struct Config
  {
    ...
    float loop_rate = 0.0;
    std::string device = "";       // SPI device (e.g., /dev/spidev0.0)
    int spi_speed_hz = 500000;     // Default SPI speed
...
private:
  SPIComms comms_;       // Replaced ArduinoComms with SPIComms
  Config cfg_;
  Wheel wheel_l_;
  Wheel wheel_r_;
};
